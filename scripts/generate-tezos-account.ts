...
async function generateTezosAccount(): Promise<void> {
  ...
  // Convert Buffer to Uint8Array and encode as edsk2 (Tezos ed25519 32-byte seed key)
  // Note: EDSK2 is for 32-byte seeds, EDSK is for 64-byte full secret keys
  const keyUint8 = new Uint8Array(key);
  const secretKeyEncoded = b58cencode(keyUint8, prefix[Prefix.EDSK2]);
