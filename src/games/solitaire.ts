...
export class Solitaire extends CardGame {
  ...
  getMinPlayers() {
    return Solitaire.requiredPlayers;
  }
  getMaxPlayers() {
    return Solitaire.requiredPlayers;
  }
  ...
  async begin(initialSetup: boolean) {
    ...
    const foundations = await Promise.all(
      Array(4)
        .fill(null)
        .map((_, i) => initDeck(this.tableId, getDeckName(0.06, -0.14 - i * 0.09)))
    );
    const tableau = await Promise.all(
      Array(7)
        .fill(null)
        .map((_, i) => initDeck(this.tableId, getDeckName(-0.1, 0.25 - i * 0.09)))
    );
