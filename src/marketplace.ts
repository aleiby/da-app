...
*/
      ...
      const batch = Tezos.contract
        ...
        .withContractCall(
          fa2.methodsObject.update_operators(
            result.tokenIds.map((id: number) => ({
              add_operator: { operator: escrowContract, token_id: id, owner: adminAddress },
            }))
          )
        )
        .withContractCall(
          escrow.methodsObject.redeem_funds({
            to: purchaserAddress,
            ids: result.tokenIds,
            amount: escrowAmount,
          })
      ...
      result = await collection.findOneAndUpdate(
        ...
        {
          $set: {
            sold: { date: Date(), hash: batchOp.hash, to: purchaserAddress, amount: escrowAmount },
          },
          $unset: { pending: 0 },
        },
        { returnDocument: ReturnDocument.AFTER }
