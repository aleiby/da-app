...
export class Connection {
  ...
  async setName(name: string) {
    ...
    if (this.tableId) {
      sendEvent(this.tableId, 'nameChanged', await getPlayerSlot(this.tableId, this.userId));
    }
  }
  ...
  welcome(name: string) {
    ...
    if (isDevelopment) {
      msg += ` (Server: ${path.dirname(__dirname)})`;
    }
  ...
  constructor(socket: Socket) {
    ...
    (async () => {
      ...
      while (this.connected) {
        const streams = [`${this.userId}:events`, `${this.tableId}:events`, `${this.tableId}:chat`];
        ...
        if (response && Array.isArray(response)) {
          for (const streamResult of response) {
            if (
              ...
              'name' in streamResult &&
              'messages' in streamResult
            ) {
              const { name, messages } = streamResult as {
                name: string;
  ...
  async setTable(tableId: string) {
    ...
    redis
      .zRange(`${tableId}:${this.userId}:cards`, 0, -1)
      .then(
        (cards) =>
          cards &&
          getCards(cards.map(Number)).then((cards) => this.socket.emit('revealCards', cards))
      );
  }
  ...
  handleEvent(msg: string) {
    ...
    switch (data.event) {
      case '':
        return; // filter empty messages (used to unblock msg handler in setWallet)
      case 'setTable':
        this.setTable(data.args[0]);
        return;
      case 'revealCards': {
        const cards = data.args[0] as Card[];
        redis.zAdd(
          `${this.tableId}:${this.userId}:cards`,
          cards.map((card) => ({ score: card.id, value: String(card.id) }))
        );
        break;
      }
    }
