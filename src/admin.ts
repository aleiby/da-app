...
export const adminAddress = 'tz1ii8yaEJFXMM5czchR9AwkhjG5ysTeuHbD';
  ...
  try {
    ...
    const lots = ((num = 4) => {
      ...
      for (let i = 0; i < num; i++) {
        let name: string;
        do {
          name = String.fromCharCode(ch(), ch(), ch(), ch());
    ...
    for (let lotIndex = 1; lotIndex < lotSizes.length; lotIndex++) {
      const odds: number[] = [];
      const cumulativeOdds: number[] = [];
      const selectOdds = randrange(900, 1000, rand);
      let total = 0;
      for (let i = 0; i < totalCards; i++) {
    ...
    (() => {
      let lotsTotal = 0;
      for (const lotTotals of lotsTotals) {
        const lotTotal = lotTotals.reduce((cumulative: number, n: number) => cumulative + n);
