...
const getCardStates = async (key: string, idStrings: string[]) => {
  const facingStrings = idStrings.length > 0 ? await redis.hmGet(`${key}:facing`, idStrings) : [];
  return idStrings.map((idString, i) => ({
    id: Number(idString),
    facing: Number(facingStrings[i]),
  }));
};
...
export const initDeck = async (tableId: string, name: string) => {
  ...
  const [minScore, maxScore] =
    cards.length > 0
      ? [Math.min(-1, cards[0].score), Math.max(1, cards[cards.length - 1].score)]
      : [-1, 1]; // Reserve zero for not in deck (default returned by zScore).
...
export class CardDeck {
  ...
  _addIds(idStrings: string[], toStart = false) {
    ...
    redis.zAdd(
      this.key,
      idStrings.map((idString) => ({ score: i++, value: idString }))
    );
  }
  ...
  move = (cards: Card[], to: CardDeck, toStart = false) =>
    this.moveIds(
      cards.map((card) => card.id),
      to,
      toStart
  ...
  flipIds(ids: number[]) {
    ...
    redis.hmGet(key, idStrings).then((facingStrings) =>
      sendEvent(
        ...
        'facing',
        this.key,
        facingStrings.map((facingString, i) => ({ id: ids[i], facing: Number(facingString) }))
      )
    );
  }
...
export const getShuffledDeck = async (walletAddress: string, contents = DeckContents.AllCards) => {
  ...
  // Check ownership for all values first
  const ownershipChecks = await Promise.all(
    values.map(async (value) => {
      const best = await getBestOwned(walletAddress, value);
      return { value, owned: best };
    })
  );

  // Separate into owned and loaner values
  const ownedChecks = ownershipChecks.filter((c) => c.owned !== null);
  const loanerValues = ownershipChecks.filter((c) => c.owned === null).map((c) => c.value);

  // Shuffle each group separately using indices
  const ownedIndices = Array.from({ length: ownedChecks.length }, (_, i) => i);
  shuffle(ownedIndices);
  shuffle(loanerValues);

  // Build deck: owned cards first, then loaners
  const ownedCards = ownedIndices.map((i) => ownedChecks[i].owned!);
  const loanerCards = await Promise.all(loanerValues.map((value) => registerCard(value)));

  return [...ownedCards, ...loanerCards];
};
